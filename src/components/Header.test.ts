...
const links = [
  { label: 'Email', href: 'mailto:nicolas.nisoria@gmail.com', icon: 'email' as const },
  { label: 'LinkedIn', href: 'https://linkedin.com/in/niconisoria', icon: 'linkedin' as const },
  { label: 'GitHub', href: 'https://github.com/niconisoria', icon: 'github' as const },
...
function anchorFor(html: string, label: string) {
  const match = html.match(new RegExp(`<a[^>]*>(?:(?!</a>).)*?${label}(?:(?!</a>).)*?</a>`, 's'));
  return match?.[0] ?? '';
}
...
describe('Header', () => {
  ...
  it('renders a subtitle when provided, omits it when not', async () => {
    ...
    const withSubtitle = await container.renderToString(Header, {
      props: { name: 'Nicolas Nisoria', subtitle: 'Senior Software Engineer', links: [] },
    });
  ...
  it('renders with no links passed', async () => {
    const container = await AstroContainer.create();
    const html = await container.renderToString(Header, { props: { name: 'Nicolas Nisoria' } });
    expect(html).toContain('Nicolas Nisoria');
  });
});
