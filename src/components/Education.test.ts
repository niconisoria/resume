...
describe('Education', () => {
  ...
  it('highlights the institution differently from the degree', async () => {
    const html = await render();
    const degreeMatch = html.match(/<[^>]*>MSc Systems Engineering<\/[^>]*>/)?.[0] ?? '';
    const institutionMatch = html.match(/<[^>]*>Universidad Tecnológica Nacional<\/[^>]*>/)?.[0] ?? '';
    const degreeClass = degreeMatch.match(/class="([^"]*)"/)?.[1] ?? '';
    const institutionClass = institutionMatch.match(/class="([^"]*)"/)?.[1] ?? '';
  ...
  it('renders multiple entries when given', async () => {
    ...
    const html = await container.renderToString(Education, {
      props: {
        items: [
          ...items,
          { degree: 'BSc Computer Science', institution: 'Some University', startDate: '2010', endDate: '2014' },
        ],
      },
    });
