...
const props = {
  ...
  selectedWork: [
    { label: 'SCIM Bridge', href: 'https://github.com/niconisoria/scim-bridge' },
...
describe('Sidebar', () => {
  ...
  it('keeps selected-work "#" links inert while opening real links in a new tab', async () => {
    ...
    const realLinkAnchor = workBlock.match(/<a[^>]*>(?:(?!<\/a>).)*?SCIM Bridge(?:(?!<\/a>).)*?<\/a>/s)?.[0] ?? '';
    expect(realLinkAnchor).toContain('target="_blank"');

    const placeholderAnchor = workBlock.match(/<a[^>]*>(?:(?!<\/a>).)*?Internal tool(?:(?!<\/a>).)*?<\/a>/s)?.[0] ?? '';
