...
Constraints: reuse existing `SkillList` (already spec'd: `{label, years?, caption?}` items, no pill/badge). No `Testimonial` (dropped earlier). Colors from registered Tailwind tokens (navy-_, ink-_, paper, wood-500), no new arbitrary hex.
